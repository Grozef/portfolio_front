    ...
    <main v-else class="projects-main">
      <div ref="scrollContainer" class="projects-scroll" @scroll="handleScroll">
        <article v-for="(project, index) in projects" :key="project.id" class="project-card"
          :class="{ active: activeIndex === index }" @click="handleOpenProject(project)" data-cursor-hover>
      ...
      <div class="scroll-nav">
        <button class="nav-arrow nav-arrow-prev" :class="{ disabled: activeIndex === 0 }" @click="goToPrevProject"
          :disabled="activeIndex === 0" data-cursor-hover title="Previous project">
          <span>←</span>
        </button>

        <div class="nav-dots-container">
          <button v-for="(project, index) in projects.slice(0, 10)" :key="project.id" class="nav-dot"
            :class="{ active: activeIndex === index }" @click="scrollToProject(index)" data-cursor-hover></button>
        ...
        <button class="nav-arrow nav-arrow-next" :class="{ disabled: activeIndex === projects.length - 1 }"
          @click="goToNextProject" :disabled="activeIndex === projects.length - 1" data-cursor-hover
          title="Next project">
          <span>→</span>
        </button>
      </div>
...
const handleScroll = () => {
  ...
  if (scrollPosition + containerWidth >= scrollWidth - 500) {
